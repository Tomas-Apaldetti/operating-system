...
umain(int argc, char **argv)
{
    ...
    cprintf("Then I changed my priority to something with lower priority (%d) \n", prio);

    r = sys_set_prio(me, prio);

    if(r != 0)
    {
        cprintf("Could change my priority to something with more priority.\n");
        cprintf("Shutting down\n");
        return;
    }
    ...
    if (pid < 0 ){
        ...
        me = sys_getenvid();
        prio = sys_get_prio(me);
        cprintf("I am the child starting with less priority %d\n", prio);
    }
